...
public class _01_IntroToQueue {
	public static void main(String[] args) {
		...
		for (int i = 1; i <= r.nextInt(5); i++) {
			Double removed = cue.remove();
			System.out.println("removing " + i + " elements from Queue: " + removed);
		}
		...
		while (dubble.size() != 0) {
			Double removed = dubble.pop();
			System.out.println(removed);
			
		}
		System.out.println(dubble.size() + " = Double size");
		System.out.println(cue.size()+ " Queue size");
	}
}
